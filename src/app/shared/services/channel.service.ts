...
export class ChannelService implements OnDestroy {
  ...
  public listenToChannelUpdates(): void {
    ...
    const unsubscribePublic = onSnapshot(publicChannelsQuery, (snapshot) => {
      ...
      this.cacheService.set('channels-public', channels); // Echtzeitdaten, keine TTL
    }, (error) => {
    ...
    const unsubscribePrivate = onSnapshot(privateChannelsQuery, (snapshot) => {
      const channels: Channel[] = snapshot.docs.map(docSnap => ({ ...docSnap.data(), id: docSnap.id } as Channel));
      this.cacheService.set('channels-private', channels); // Echtzeitdaten, keine TTL
    }, (error) => {
  ...
  public listenToChannelMessages(channelId: string, isPrivate: boolean): void {
    ...
    const unsubscribe = onSnapshot(
      ...
      (snapshot) => {
        ...
        this.cacheService.set(key, messages); // Echtzeitdaten, keine TTL
      },
      (error) => {
        console.error(
  ...
  async getChannelByName(
    ...
    if (!querySnapshot.empty) {
      ...
      this.cacheService.set(key, channel); // Echtzeitdaten, keine TTL
      return channel;
    } else {
      this.cacheService.set(key, null); // Echtzeitdaten, keine TTL für nicht gefundene Kanäle
      return null;
    }
  }
